...
import com.amazonaws.services.costexplorer.{AWSCostExplorer, AWSCostExplorerClientBuilder}
...
abstract class AbstractCostExplorerOperator(operatorName: String, context: OperatorContext, systemConfig: Config, templateEngine: TemplateEngine)
  ...
  protected val secrets: SecretProvider = context.getSecrets.getSecrets("cost_explorer")

  protected val sessionUuid: String = params.get("session_uuid", classOf[String])

  protected val isAllowedAuthMethodEnv: Boolean = systemConfig.get("cost_explorer.allow_auth_method_env", classOf[Boolean], false)
  protected val isAllowedAuthMethodInstance: Boolean = systemConfig.get("cost_explorer.allow_auth_method_instance", classOf[Boolean], false)
  protected val isAllowedAuthMethodProfile: Boolean = systemConfig.get("cost_explorer.allow_auth_method_profile", classOf[Boolean], false)
  protected val isAllowedAuthMethodProperties: Boolean = systemConfig.get("cost_explorer.allow_auth_method_properties", classOf[Boolean], false)
  protected val assumeRoleTimeoutDuration: DurationParam =
    systemConfig.get("cost_explorer.assume_role_timeout_duration", classOf[DurationParam], DurationParam.parse("1h"))
  ...
  protected val roleSessionName: String = secrets.getSecretOptional("role_session_name").or(s"digdag-cost_explorer-$sessionUuid")
  ...
  protected def withAWSCostExplorer[T](f: AWSCostExplorer => T): T = {
    val ce = buildAWSCostExplorer
    try f(ce)
    finally ce.shutdown()
  }

  private def buildAWSCostExplorer: AWSCostExplorer = {
    val builder = AWSCostExplorerClientBuilder
      .standard()
  ...
  private def clientConfiguration: ClientConfiguration = {
    ...
    val protocol: Protocol = httpProxy.getSecretOptional("scheme").or("https") match {
      ...
      case _ => throw new ConfigException(s"""[$operatorName] `cost_explorer.http_proxy.scheme` must be "http" or "https".""")
    }
